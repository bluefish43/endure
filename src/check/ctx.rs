...
use crate::ast::{expr::BinaryOp, typing::{NameIndex, PrimType, Type}, Argument, Collection, Identifier, Loc, Prototype};

use super::{namespaces::*, scopes::{Scope, Scopes, Variable}, ActFunction, CheckerError, Function, RequiredState};
...
impl Context {
    ...
    pub fn lookup_any(
        ...
        if let Some(var) = self.lookup_variable(name).map(|var| SearchItem::Variable(var)) {
            Some(var)
        } else {
            self.lookup_member(name).map(|member| SearchItem::Member(member))
        }
    }
    ...
    pub fn add_function_scope(
        ...
        self.call_stack.push_scope(
            Some(
                self.current_function.clone().expect("Adding function scope without having the current function set")
            ),
            unsafety,
        );
    ...
    pub fn restart_child_namespace(
        ...
        let namespace = self
            .find_child_of_current_namespace(
                name
            )?
            .upgrade()
            .unwrap();
        ...
        match (left, right) {
            (Type::NamedType(left_name), Type::NamedType(right_name)) => {
                ...
                let left_member = match self.lookup_member(&left_name.index()) {
                    Some(member) => {
                        // TODO: Implement named type comparison
                        unreachable!()
                    }
                    None => {
                        errors.push(CheckerError::MemberNotFound(
                            at.clone(),
                            left_name.index()
                        ))
                    }
    ...
    pub fn type_supports_bin_op(
        &self,
        ty: &Type,
        operator: &BinaryOp
    ) -> bool {
        match ty {
            Type::Primitive { loc: _, ty } => {
                match operator {
                    BinaryOp::Plus => {
                        return matches!(
                            ty,
    ...
    pub fn declare_slot_in_current_scope(&mut self, name: NameIndex, ty: Type) {
        self.call_stack.last_scope_mut()
            .unwrap()
            .insert_variable(name, ty, false);
    }

    /// Gets the current namespace.
    fn current_namespace(&self) -> &ActNamespace {
        let current_namespace = self.current_namespaces
            .last()
            .unwrap();
        current_namespace
    }
